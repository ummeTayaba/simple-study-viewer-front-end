...
const StudyCreateEditValidationSchema = Yup.object().shape({
  studyName: Yup.string().max(50, 'Too Long!').required('Required'),
  studyDescription: Yup.string().max(200, 'Too Long!'),
...
const StudyForm = props => {
  ...
  const requestURL = editStudyData
    ? `${process.env.REACT_APP_API_BASE_URL}/studies/${editStudyData.id}`
    ...
    <SimpleGrid columns={1} spacingX="40px" spacingY="20px" m={10}>
      ...
      <Formik
        initialValues={{
          ...
          patientId: editStudyData
            ? editStudyData.patient.id
            : patients.length > 0
            ? patients[0].id
            : null,
        }}
          ...
          <Form onSubmit={handleSubmit}>
            <Flex>
              <Field name="studyName">
                {({ field, form }) => (
                  <FormControl
                    isInvalid={form.errors.studyName && form.touched.studyName}
                  >
              ...
              <Field name="studyDescription">
                {({ field, form }) => (
                  <FormControl
                    isInvalid={
                      form.errors.studyDescription &&
                      form.touched.studyDescription
                    }
                  >
                    <FormLabel htmlFor="studyDescription">
                      Study Description{' '}
                    </FormLabel>
                    <Input
                      {...field}
...
const StudyEdit = props => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_BASE_URL}/studies/${props.id}`)
      .then(response => {
  ...
  return (
    <StudyForm
      patients={apiResponse.patientListData}
      study={apiResponse.study}
    />
  );
};
...
const StudyCreateNew = () => {
  ...
  return <StudyForm patients={apiResponse.patientListData} />;
};
